...
import { ProposalPlaceInterface } from "./placeInterface";

export async function getProposalPlaces(): Promise<ProposalPlaceInterface[]> {
  try {
    const querySnapshot = await db.collection("proposalPlaces").get();
    const data = querySnapshot.docs.map((doc) => {
      const item = doc.data() as ProposalPlaceInterface;
      item.id = doc.id;
      return item;
    });
    return data;
